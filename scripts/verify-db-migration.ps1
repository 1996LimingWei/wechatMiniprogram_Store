...
try {
    ...
    Assert-Equal (Invoke-Mysql $TestDatabase "SELECT COUNT(*) FROM schema_migration_history WHERE version = '20260816_01';") 1 "应执行素材库结构与权限迁移"
    Assert-Equal (Invoke-Mysql $TestDatabase "SELECT COUNT(*) FROM schema_migration_history WHERE version = '20260816_02';") 1 "应执行商品导入导出 SKU 编码迁移"
    Assert-Equal (Invoke-Mysql $TestDatabase "SELECT COUNT(*) FROM information_schema.columns WHERE table_schema = DATABASE() AND table_name = 'product_sku' AND column_name = 'sku_code';") 1 "商品 SKU 应包含运营编码字段"
    Assert-Equal (Invoke-Mysql $TestDatabase "SELECT COUNT(*) FROM schema_migration_history WHERE version = '20260816_03';") 1 "应执行库存工作台预警库存迁移"
    ...
    Assert-Equal (Invoke-Mysql $TestDatabase "SELECT COUNT(*) FROM schema_migration_history WHERE version = '20260816_06';") 1 "应执行支付异常工作台迁移"
    Assert-Equal (Invoke-Mysql $TestDatabase "SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = DATABASE() AND table_name = 'pay_exception';") 1 "应创建支付异常表"
    Assert-Equal (Invoke-Mysql $TestDatabase "SELECT COUNT(*) FROM information_schema.columns WHERE table_schema = DATABASE() AND table_name = 'pay_order' AND column_name IN ('wechat_trade_state','wechat_amount','sync_message');") 3 "支付单应记录最近微信查单状态"
    Assert-Equal (Invoke-Mysql $TestDatabase "SELECT COUNT(*) FROM sys_role WHERE code = 'FINANCE' AND status = 1 AND deleted = b'0';") 1 "应创建财务角色"
    Assert-Equal (Invoke-Mysql $TestDatabase "SELECT COUNT(*) FROM sys_role_permission rp JOIN sys_role r ON r.id = rp.role_id JOIN sys_permission p ON p.id = rp.permission_id WHERE r.code = 'FINANCE' AND p.code IN ('trade:payment-read','trade:payment-sync','trade:payment-handle');") 3 "财务角色必须具备支付异常工作台权限"
    Assert-Equal (Invoke-Mysql $TestDatabase "SELECT COUNT(*) FROM schema_migration_history WHERE version = '20260816_07';") 1 "应执行退款异常工作台迁移"
    Assert-Equal (Invoke-Mysql $TestDatabase "SELECT COUNT(*) FROM information_schema.columns WHERE table_schema = DATABASE() AND table_name = 'trade_after_sale' AND column_name IN ('refund_channel_state','refund_exception_code','refund_exception_message','refund_handled','refund_handle_remark','refund_handle_admin_id','refund_handle_time');") 7 "售后表应记录退款异常状态与处理记录"
    Assert-Equal (Invoke-Mysql $TestDatabase "SELECT COUNT(*) FROM sys_role_permission rp JOIN sys_role r ON r.id = rp.role_id JOIN sys_permission p ON p.id = rp.permission_id WHERE r.code = 'FINANCE' AND p.code IN ('trade:refund-read','trade:refund-sync','trade:refund-retry','trade:refund-handle');") 4 "财务角色必须具备退款异常工作台权限"
    Assert-Equal (Invoke-Mysql $TestDatabase "SELECT COUNT(*) FROM schema_migration_history WHERE version = '20260816_08';") 1 "应执行日终对账工作台迁移"
    Assert-Equal (Invoke-Mysql $TestDatabase "SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = DATABASE() AND table_name IN ('trade_reconcile_batch','trade_reconcile_difference');") 2 "应创建日终对账批次与差异表"
    Assert-Equal (Invoke-Mysql $TestDatabase "SELECT COUNT(*) FROM sys_role_permission rp JOIN sys_role r ON r.id = rp.role_id JOIN sys_permission p ON p.id = rp.permission_id WHERE r.code = 'FINANCE' AND p.code IN ('trade:reconcile-read','trade:reconcile-trigger','trade:reconcile-export','trade:reconcile-handle');") 4 "财务角色必须具备日终对账工作台权限"
    Assert-Equal (Invoke-Mysql $TestDatabase "SELECT COUNT(*) FROM schema_migration_history WHERE version = '20260816_09';") 1 "应执行 v1.0 角色权限矩阵迁移"
    Assert-Equal (Invoke-Mysql $TestDatabase "SELECT COUNT(*) FROM sys_role WHERE code = 'READONLY' AND status = 1 AND deleted = b'0';") 1 "应创建只读角色"
    Assert-Equal (Invoke-Mysql $TestDatabase "SELECT COUNT(*) FROM sys_role_permission rp JOIN sys_role r ON r.id = rp.role_id JOIN sys_permission p ON p.id = rp.permission_id WHERE r.code = 'READONLY' AND ((p.http_method <> 'GET' AND p.code <> 'auth:session') OR p.code IN ('trade:order-export','trade:order-export-sensitive','trade:reconcile-export'));") 0 "只读角色不得具备业务写权限或敏感导出权限"
    Assert-Equal (Invoke-Mysql $TestDatabase "SELECT COUNT(*) FROM sys_role_permission rp JOIN sys_role r ON r.id = rp.role_id JOIN sys_permission p ON p.id = rp.permission_id WHERE r.code = 'PRODUCT_OPERATOR' AND p.code LIKE 'trade:reconcile%';") 0 "商品运营不得具备财务对账权限"
    Assert-Equal (Invoke-Mysql $TestDatabase "SELECT COUNT(*) FROM sys_role_permission rp JOIN sys_role r ON r.id = rp.role_id JOIN sys_permission p ON p.id = rp.permission_id WHERE r.code = 'ORDER_CUSTOMER_SERVICE' AND p.code = 'product:manage';") 0 "客服不得具备商品管理权限"
    Assert-Equal (Invoke-Mysql $TestDatabase "SELECT COUNT(*) FROM sys_role_permission rp JOIN sys_role r ON r.id = rp.role_id JOIN sys_permission p ON p.id = rp.permission_id WHERE r.code = 'FINANCE' AND p.code IN ('trade:order-ship','trade:order-batch-ship');") 0 "财务不得具备发货权限"
    Assert-Equal (Invoke-Mysql $TestDatabase "SELECT COUNT(*) FROM schema_migration_history WHERE version = '20260816_10';") 1 "应执行高风险操作审计字段迁移"
    Assert-Equal (Invoke-Mysql $TestDatabase "SELECT COUNT(*) FROM information_schema.columns WHERE table_schema = DATABASE() AND table_name = 'sys_operation_log' AND column_name IN ('admin_role_codes','operation_type','high_risk','user_agent','before_snapshot','after_snapshot');") 6 "操作审计应具备角色、类型、高风险标记、客户端和变更摘要字段"
    Assert-Equal (Invoke-Mysql $TestDatabase "SELECT COUNT(DISTINCT index_name) FROM information_schema.statistics WHERE table_schema = DATABASE() AND table_name = 'sys_operation_log' AND index_name IN ('idx_operation_type_time','idx_high_risk_time');") 2 "操作审计应支持按操作类型和高风险标记查询"
    Assert-Equal (Invoke-Mysql $TestDatabase "SELECT COUNT(*) FROM schema_migration_history WHERE version = '20260816_11';") 1 "应执行可观测性与告警迁移"
    Assert-Equal (Invoke-Mysql $TestDatabase "SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = DATABASE() AND table_name IN ('sys_observability_alert','sys_job_execution_metric');") 2 "应创建运行告警与任务指标表"
    Assert-Equal (Invoke-Mysql $TestDatabase "SELECT COUNT(*) FROM sys_permission WHERE code = 'trade:observability-read' AND path_pattern = '/admin-api/trade/observability/**' AND status = 1 AND deleted = b'0';") 1 "应登记运行监控后台权限"
    Assert-Equal (Invoke-Mysql $TestDatabase "SELECT COUNT(*) FROM schema_migration_history WHERE version = '20260816_12';") 1 "应执行退款通知审计迁移"
    Assert-Equal (Invoke-Mysql $TestDatabase "SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = DATABASE() AND table_name IN ('refund_notify_log','refund_notify_failure_log');") 2 "应创建退款通知流水和失败审计表"
    Assert-Equal (Invoke-Mysql $TestDatabase "SELECT COUNT(*) FROM information_schema.statistics WHERE table_schema = DATABASE() AND table_name = 'refund_notify_log' AND index_name = 'uk_refund_notification_id';") 1 "退款通知流水必须按通知 ID 幂等"
