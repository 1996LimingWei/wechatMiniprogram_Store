...
try {
    ...
    Assert-Equal (Invoke-Mysql $TestDatabase "SELECT COUNT(*) FROM information_schema.statistics WHERE table_schema = DATABASE() AND table_name = 'sys_operation_log' AND index_name IN ('idx_operation_type_time','idx_high_risk_time');") 2 "操作审计应支持按操作类型和高风险标记查询"
