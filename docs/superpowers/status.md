...
生产交易闭环代码已完成第二轮收口，下一步按上线环境执行：
1. 启动 Docker Desktop，在隔离数据库执行 `V20260807_01` 至 `V20260808_02` 增量迁移并构建后台 Nginx 镜像。
2. 使用微信商户测试配置联调 JSAPI 下单、支付回调丢失主动查单、订单关闭、退款成功与退款异常。
3. 将当前单账号明文管理员认证升级为数据库账号、密码哈希、失败限流和审计锁定。
