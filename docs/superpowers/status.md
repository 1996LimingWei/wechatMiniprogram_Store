...
非会员域代码整改与本地验收已完成，下一步按企业上线环境执行：
1. 注入生产数据库、Redis、管理员强密码、微信支付和快递 100 凭据，执行全部迁移并验证生产启动门禁。
2. 使用真实小额订单联调 JSAPI 支付、回调丢失主动查单、超时关单、仅退款、退货退款和物流轨迹。
3. 根据客户履约规则确认是否进入多包裹、区域运费模板、仓库/WMS/ERP 对接阶段；会员域继续保持暂缓。
